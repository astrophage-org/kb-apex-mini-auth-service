...
## Goal
Protect user accounts against brute-force and credential-stuffing attacks on the authentication endpoint (`POST /api/v1/auth/login`) as documented in [[kb:mini-auth-service/summaries/api-reference]] by temporarily locking an account for 15 minutes after 5 consecutive failed password attempts.
...
- **When** a client submits invalid credentials for a username to `POST /api/v1/auth/login` via `OAuth2PasswordRequestForm` as documented in [[kb:mini-auth-service/summaries/api-reference]],
...
- **Then** the account is placed in a locked state for a duration of 15 minutes and the login attempt is rejected with an HTTP 401 Unauthorized response indicating account lockout.
...
- **Then** the service rejects the authentication request with an HTTP 401 Unauthorized response and denies access token generation without resetting the remaining lockout timer.
...
- **Then** the service returns a 200 OK response with a valid signed Bearer JWT and immediately resets the consecutive failed login attempt counter for that username back to 0.

### AC-5: Automatic unlock after lockout expiration
- **Given** an account that has been locked for at least 15 minutes,
- **When** the user submits valid credentials to `POST /api/v1/auth/login`,
- **Then** the lock is treated as expired, the login succeeds with an HTTP 200 OK response containing a signed Bearer JWT as defined in [[kb:mini-auth-service/concepts/jwt-authentication-flow]], and the failure counter is reset to 0.

### AC-6: Failed attempt after lockout expiration
- **Given** an account that has been locked for at least 15 minutes,
- **When** the user submits invalid credentials to `POST /api/v1/auth/login`,
- **Then** the expired lockout is cleared, the failed attempt counter is set to 1, and an HTTP 401 Unauthorized response is returned.

## Edge cases
- **Non-existent usernames *(from the map)*:** When failed login attempts are made against non-existent usernames in the authentication store during the [[kb:mini-auth-service/concepts/jwt-authentication-flow|JWT authentication flow]], the system must handle attempt tracking safely without unbounded memory leaks and without leaking whether an account exists.
...
- **Existing active JWT tokens during lockout *(from the map)*:** Previously issued, unexpired JWT tokens verified by [[kb:mini-auth-service/concepts/token-verification-guard|Token Verification Guard]] on `GET /api/v1/auth/me` remain valid until their expiration; lockout strictly governs new token issuance on `POST /api/v1/auth/login`.
...
- Modifications to token inspection on `GET /api/v1/auth/me` or `/auth/me`.
...
- [ ] AC-6: Submitting invalid credentials after the 15-minute lockout duration resets the counter to 1 failed attempt.
...
- [ ] Edge Case: Existing issued tokens on `GET /api/v1/auth/me` continue functioning per [[kb:mini-auth-service/concepts/token-verification-guard]] without interruption *(from the map)*.
